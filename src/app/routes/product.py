...
def search_page():
    ...
    filters = {
        ...
        "brand": [request.args.get('brand', None), lambda brand, query: query.filter(Product.producer.match(f"%{brand}%"))],
    ...
    print(list(product.price_history))

    # priceh = PriceHistory(19, 300)

    # db.session.add(priceh)
    # db.session.commit()
